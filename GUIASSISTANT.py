...
def main(text):
		...
		if isContain(text, ['spotify']):
			speak('Opening...', True, True)
			webScrapping.openWebsite('https://www.spotify.com/in-en/')
		
		if isContain(text, ['facebook']):
			speak('Opening...', True, True)
			webScrapping.openWebsite('https://www.facebook.com/')
		...
		if isContain(text, ['game']):
			...
			if 'online' in text:
				...
				webScrapping.openWebsite('https://www.miniclip.com/games/en/')
				return
...
if __name__ == '__main__':
	...
	splash_label = Label(splash_root, text="Loading....", font=('montserrat',15),bg='#3895d3',fg='white')
	...
	#User and Bot Icon
	userIcon = PhotoImage(file="extrafiles/images/user.png")
